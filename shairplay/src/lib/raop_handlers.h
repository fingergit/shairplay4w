...
static void
raop_handler_setup_airplay(raop_conn_t* conn,
	http_request_t* request, http_response_t* response,
	char** response_data, int* response_datalen)
{
	...
	if (setup == 0) {
		...
		conn->raop_rtp = raop_rtp_init_airplay(conn->raop->logger, &conn->raop->callbacks, conn->remote, conn->remotelen, aeskey, aesiv, ecdh_secret, timing_rport);
